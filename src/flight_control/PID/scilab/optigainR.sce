...
// z1,  z2 are REAL zeros of PID  controller
...
// Increments
dz1  = (z1max-z1min)/nvals;
dz2  = (z2max-z2min)/nvals;
...
///////////////////////////////  Main Loop ////////////////////////////////
for z1 = z1min:dz1:z1max, 
    printf(".");
    for z2 = z2min:dz2:z2max, 
        for Kd = kdmin:dkd:kdmax,
       //     printf("%4d: K: %3.1f %3.1f %3.1f  emin(1): %f ", iter, Kp, Ki, Kd, emin(1));
            iter = iter + 1;
            
            Ki =   z1*z2/Kd;   // derive Kp, Ki from the real zeros
            Kp =  -1*(z1+z2)/Kd;
...
/////////////////////////////////////////   Print Results 
for i=1:Ns, 
   if(Kpo(i) < .01 | Kio(i) < 0.01 | Kdo(i) < 0.01) then {printf("\n\n[%15s] Kp: %12.9f  Ki: %12.9f  Kd: %12.9f  z1: %12.9f  z2:  %12.9f\n",wnames(i), Kpo(i),Kio(i),Kdo(i), z1o(i), z2o(i)); };
      else {  printf("\n\n[%15s] Kp: %6.3f  Ki: %6.3f  Kd: %6.3f  z1: %6.3f  z2:%6.3f\n",wnames(i), Kpo(i),Kio(i),Kdo(i), z1o(i), z2o(i)); } ;   
   end
   ...
   if( Kdo(i) == kdmin | Kdo(i) == kdmax | z1o(i) == z1min |  z1o(i) == z1max |  z2o(i) == z2min | z2o(i) ==  z2max)  then 
       printf("  Search boundary reached:  "); 
       if  Kdo(i) == kdmin then printf("Kd min "); end
       if  Kdo(i) == kdmax then printf("Kd max "); end
       if  z1o(i) == z1max then printf("z1 max "); end
       if  z1o(i) == z1min then printf("z1 min "); end
       if  z2o(i) == z2min then printf("z2 min "); end
       if  z2o(i) == z2max then printf("z2 max "); end
       printf("\n");  
   end
 end
 ...
 printf("\nSearch Time:  %3.1f minutes.  N = %d  (%8.1f sims/min)\n", (end_time-start_time)/60, (nvals+1)^3, (nvals+1)^3 / ((end_time-start_time)/60));
