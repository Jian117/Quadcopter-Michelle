...
{   
   ...
   // init system modules
   stabilize_init(&kinetic_errors_struct, &(roll_PID_struct), &(pitch_PID_struct), &(yaw_PID_struct));
   uart_init();
   timer_init();
   controller_init(&(quadcopter_intrinsics_struct), &(controller_data_struct), &(controller_param_struct), &(test_motor_speed[0]));
   motors_init();
   interrupt_init();
